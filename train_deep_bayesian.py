#!/usr/bin/env python3
"""
Training script for Deep Bayesian-DKVMN GPCM model.

This script trains the deeply integrated Bayesian-DKVMN architecture where
IRT parameters are embedded in the memory network operations themselves.
"""
...
def compute_parameter_recovery_metrics(true_params: Dict[str, np.ndarray],
    ...
    # Alpha comparison (use first few memory keys as question representatives)
    ...
    # Beta comparison (compare mean thresholds)
    ...
    # Theta comparison (use first few memory values as student representatives)
    n_students = min(len(true_params['theta']), len(learned_theta))
    # Take mean across value dimensions for learned theta
    learned_theta_mean = learned_theta[:n_students].mean(axis=1)
    theta_corr = np.corrcoef(true_params['theta'][:n_students], 
                           learned_theta_mean)[0, 1]
...
def main():
    parser = argparse.ArgumentParser(description='Train Deep Bayesian-DKVMN GPCM model')
    ...
    for epoch in range(args.epochs):
        ...
        if test_metrics['categorical_accuracy'] > best_test_accuracy:
            ...
            save_path = save_dir / f'best_deep_bayesian_{args.dataset}.pth'
    ...
    checkpoint = torch.load(save_dir / f'best_deep_bayesian_{args.dataset}.pth')
    ...
    history_file = log_dir / f'deep_bayesian_training_history_{args.dataset}_{timestamp}.json'
