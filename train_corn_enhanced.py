#!/usr/bin/env python3
"""
Enhanced Training Script for Deep-GPCM with CORN Integration

Supports both original GPCM and CORN-enhanced training with embedding strategy comparison.
"""
...
from models.dkvmn_corn_integration import DeepGpcmCornModel, create_corn_enhanced_model
...
def evaluate_model_enhanced(model, data_loader, loss_fn, metrics, device, n_cats, use_corn=None):
    """Enhanced evaluation supporting both GPCM and CORN models."""
    model.eval()
    total_loss = 0.0
    all_probs = []
    all_targets = []
    ...
    with torch.no_grad():
        for q_batch, r_batch, mask_batch in data_loader:
            ...
            if hasattr(model, 'forward') and 'use_corn' in model.forward.__code__.co_varnames and use_corn is not None:
                # CORN-enhanced model
                _, _, _, probs, logits = model(q_batch, r_batch, use_corn=use_corn)
            else:
                # Original model
                _, _, _, probs, logits = model(q_batch, r_batch)
            ...
            # Store for metrics
            all_probs.append(probs[mask_batch].cpu() if mask_batch is not None else probs.view(-1, n_cats).cpu())
...
def train_epoch_enhanced(model, train_loader, optimizer, loss_fn, device, n_cats, use_corn=None):
    """Enhanced training epoch supporting both GPCM and CORN models."""
    model.train()
    total_loss = 0.0
    n_batches = 0
...
def train_corn_enhanced_model(dataset_name, n_epochs=10, batch_size=64, learning_rate=0.001,
                              embedding_strategy='ordered', model_type='corn', device=None):
    """
    Train CORN-enhanced model with comprehensive benchmarking.
    ...
    Args:
        ...
        model_type: 'gpcm' for original, 'corn' for CORN-enhanced
    ...
    if model_type == 'corn':
        model = create_corn_enhanced_model(base_model)
        model = model.to(device)
        use_corn = True
        loss_fn = CornLoss(n_cats)
        print(f"Created CORN-enhanced model")
    else:
        model = base_model
        use_corn = None
    ...
    for epoch in range(n_epochs):
        ...
        # Train
        train_loss = train_epoch_enhanced(model, train_loader, optimizer, loss_fn, device, n_cats, use_corn)
        
        # Evaluate
        train_metrics = evaluate_model_enhanced(model, train_loader, loss_fn, metrics, device, n_cats, use_corn)
        test_metrics = evaluate_model_enhanced(model, test_loader, loss_fn, metrics, device, n_cats, use_corn)
...
def run_comprehensive_corn_embedding_comparison(dataset_name="synthetic_OC", epochs=10):
    """
    Run comprehensive comparison of GPCM vs CORN across all embedding strategies.
    """
    print(f"\n{'='*100}")
    print(f"COMPREHENSIVE CORN vs GPCM EMBEDDING STRATEGY COMPARISON")
    print(f"Dataset: {dataset_name}")
    print(f"{'='*100}")
    ...
    for embedding_strategy in embedding_strategies:
        for model_type in model_types:
            ...
            try:
                # Train model
                model, history = train_corn_enhanced_model(
    ...
    detailed_path = f"results/comparison/corn_gpcm_embedding_comparison_{dataset_name}_{timestamp}.json"
    with open(detailed_path, 'w') as f:
        json.dump({
            'experiment_info': {
                ...
                'description': 'Comprehensive CORN vs GPCM embedding strategy comparison'
            },
...
def main():
    """Main function for comprehensive CORN vs GPCM comparison."""
    parser = argparse.ArgumentParser(description='Train and compare CORN-enhanced vs GPCM models')
    ...
    if args.single:
        ...
        try:
            ...
            print("Example: --single corn,ordered")
    else:
        # Run comprehensive comparison
        run_comprehensive_corn_embedding_comparison(args.dataset, args.epochs)
