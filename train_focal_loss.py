#!/usr/bin/env python3
"""
Train Deep-GPCM using Focal Loss for handling class imbalance.
Focal loss helps focus training on hard-to-classify examples.
"""
...
def main():
    parser = argparse.ArgumentParser(description='Train Deep-GPCM with Focal Loss')
    ...
    parser.add_argument('--alpha', type=float, default=None,
                       help='Focal loss alpha parameter (if not set, computed from class frequencies)')
    ...
    # Compute class weights if alpha not provided
    ...
    # Loss and optimizer
    criterion = FocalLoss(gamma=args.gamma, alpha=alpha, reduction='none')
    ...
    early_stopping = EarlyStopping(patience=10, verbose=True, 
                                 path=f'save_models/best_deep_gpcm_focal_{args.dataset}.pth')
    
    print("\n" + "="*80)
    print("TRAINING WITH FOCAL LOSS")
    print("="*80)
    print(f"Gamma: {args.gamma}, Alpha: {alpha if isinstance(alpha, float) else 'class-weighted'}")
    print("="*80)
    ...
    save_path = save_results(
        ...
        f'results/train/focal_loss_training_{args.dataset}.json'
    )
