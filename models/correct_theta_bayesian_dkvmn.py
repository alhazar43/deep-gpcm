#!/usr/bin/env python3
"""
Correct Theta Extraction Deep Bayesian-DKVMN Implementation

This version:
1. KEEPS activation functions (they help parameter recovery!)
2. FIXES theta extraction to properly track student abilities
3. Maintains ALL deep integration functionality

Key insight: Activations are needed for IRT constraints, but theta extraction was wrong.
"""
...
class BayesianMemoryKey(nn.Module):
    ...
    KEEPS activation functions for proper IRT constraints.
    """
    ...
    def __init__(self, key_dim: int, n_categories: int = 4):
        ...
        # Discrimination parameter (log-normal) - KEEP activation for α > 0
        self.alpha_mean = nn.Parameter(torch.zeros(1))
        self.alpha_log_var = nn.Parameter(torch.ones(1) * np.log(0.3))
        
        # Threshold parameters (ordered normal) - KEEP activation for ordering
    ...
    def get_deterministic_parameters(self) -> Tuple[torch.Tensor, torch.Tensor]:
        """Get deterministic parameters (for evaluation)."""
        # Deterministic discrimination (KEEP exp)
        alpha = torch.exp(self.alpha_mean)
        
        # Deterministic ordered thresholds (KEEP softplus)
...
class BayesianMemoryValue(nn.Module):
    ...
    KEEPS activation functions for proper ability modeling.
    """
...
class StudentAbilityTracker(nn.Module):
    """
    Track individual student abilities properly.
    
    This class maintains student-specific ability tracking to fix the theta mismatch.
    """
    
    def __init__(self, max_students: int = 1000):
        super().__init__()
        self.max_students = max_students
        
        # Student ability tracking (one entry per actual student)
        self.student_abilities = nn.Parameter(torch.zeros(max_students), requires_grad=True)
        self.student_counts = nn.Parameter(torch.zeros(max_students), requires_grad=False)
        
        # Mapping from batch indices to student IDs (will be set externally)
        self.register_buffer('batch_to_student_map', torch.zeros(1, dtype=torch.long))
    
    def set_student_mapping(self, batch_to_student_ids: torch.Tensor):
        """Set mapping from batch indices to student IDs."""
        self.batch_to_student_map = batch_to_student_ids
    
    def get_student_abilities(self, batch_indices: Optional[torch.Tensor] = None) -> torch.Tensor:
        """Get abilities for students in current batch."""
        if batch_indices is None:
            # Use stored mapping
            student_ids = self.batch_to_student_map[:len(self.batch_to_student_map)]
        else:
            student_ids = batch_indices
        
        return self.student_abilities[student_ids]
    
    def update_student_abilities(self, evidence: torch.Tensor, batch_indices: Optional[torch.Tensor] = None):
        """Update student abilities based on evidence."""
        if batch_indices is None:
            student_ids = self.batch_to_student_map[:len(evidence)]
        else:
            student_ids = batch_indices
        ...
        with torch.no_grad():
            ...
            for i, student_id in enumerate(student_ids):
                if student_id < self.max_students:
...
class CorrectThetaDeepBayesianDKVMN(nn.Module):
    """
    Deep Bayesian-DKVMN with CORRECT theta extraction and KEPT activations.
    
    This version:
    1. KEEPS activation functions (they help parameter constraints)
    2. FIXES theta extraction to track actual student abilities
    3. Maintains ALL deep integration functionality
    
    Key insight: The problem was theta extraction, not activations!
    """
    
    def __init__(self, n_questions: int, n_categories: int = 4, memory_size: int = 20,
                 key_dim: int = 50, value_dim: int = 100, embed_dim: int = 64, max_students: int = 1000):
        super().__init__()
        ...
        # Bayesian Memory Network (KEEP activations)
        ...
        # GPCM prediction layers (KEEP activation for proper ability prediction)
        self.ability_predictor = nn.Linear(value_dim, 1)
        
        # Student ability tracker (FIXED theta extraction)
        self.student_tracker = StudentAbilityTracker(max_students)
    ...
    def set_student_mapping(self, batch_to_student_ids: torch.Tensor):
        """Set mapping from batch positions to student IDs."""
        self.student_tracker.set_student_mapping(batch_to_student_ids)
    ...
    def compute_attention(self, query_embed: torch.Tensor, 
        ...
        KEEPS all activation functions.
        """
        ...
        for i, memory_key in enumerate(self.memory_keys):
            ...
            # === IRT PARAMETER SIMILARITY (WITH activations) ===
            alpha_key = memory_alphas[i]
            beta_key = memory_betas[i]
            
            # Discrimination-based similarity (with exp activation)
            alpha_sim = torch.ones_like(embed_sim) * alpha_key.item()
            
            # Difficulty-based similarity (with proper activation)
    ...
    def forward(self, questions: torch.Tensor, responses: torch.Tensor,
        ...
        """
        Forward pass with CORRECT theta tracking and KEPT activations.
        """
        ...
        # Track student abilities throughout sequence
        ...
        for t in range(seq_len):
            ...
            # Extract student abilities from memory (WITH activation layer)
            predicted_abilities = self.ability_predictor(ability_mean).squeeze(-1)  # [batch_size]
            
            # Store final abilities for theta extraction
            if t == seq_len - 1:  # Last time step
                final_student_abilities = predicted_abilities.detach()
                
                # Update student tracker with final abilities
                self.student_tracker.update_student_abilities(final_student_abilities)
            
            # === COMPLETE IRT PARAMETER COMPUTATION ===
            memory_alphas, memory_betas = self.sample_memory_parameters()
            
            # Compute attention-weighted IRT parameters
            batch_probs = []
            ...
            for b in range(batch_size):
                ...
                # Get student ability from DKVMN memory read (WITH activation)
        ...
        aux_dict = {
            ...
            'final_student_abilities': final_student_abilities  # FIXED theta extraction
        }
    ...
    def get_interpretable_parameters(self) -> Dict[str, torch.Tensor]:
        ...
        with torch.no_grad():
            ...
            # Get student abilities from student tracker (FIXED theta extraction)
            student_abilities = self.student_tracker.student_abilities
            
            return {
                'alpha': torch.stack(alphas),  # [memory_size]
                'beta': torch.stack(betas),    # [memory_size, n_categories-1]
                'theta': student_abilities,    # [max_students] - FIXED!
                'memory_keys': [mk.embedding for mk in self.memory_keys]
            }
    
    def extract_all_student_abilities(self, data_loader, device):
        """Extract abilities for all students in dataset (FIXED theta extraction)."""
        self.eval()
        ...
        with torch.no_grad():
            for batch_idx, (questions, responses) in enumerate(data_loader):
                ...
                # Set student mapping for this batch
                student_ids = torch.arange(batch_idx * batch_size, 
                                         batch_idx * batch_size + batch_size)
                self.set_student_mapping(student_ids)
                
                # Forward pass to get final abilities
                _, aux_dict = self(questions, responses, return_params=True)
                
                # Extract final student abilities for this batch
                final_abilities = aux_dict['final_student_abilities']
                all_student_abilities.extend(final_abilities.cpu().numpy())
        
        return np.array(all_student_abilities)