#!/usr/bin/env python3
"""
Training script for No-Activation Deep Bayesian-DKVMN with proper theta extraction.

This version:
1. Removes ALL activation functions from IRT parameters
2. Fixes theta correlation by extracting actual student abilities 
...
def extract_student_abilities(model, data_loader, device):
    """
    Extract actual student abilities from the trained model.
    
    This fixes the theta correlation issue by getting student-specific abilities
    rather than memory slot archetypes.
    """
    model.eval()
    student_abilities = []
    ...
    with torch.no_grad():
        for questions, responses in data_loader:
            ...
            # Process each sequence to get final student ability
            for b in range(batch_size):
                # Get the sequence for this student
                q_seq = questions[b:b+1, :]  # [1, seq_len]
                r_seq = responses[b:b+1, :]  # [1, seq_len]
                
                # Run through the model to get final ability state
                _, aux_dict = model(q_seq, r_seq, return_params=True)
                
                # Get the final attention-weighted ability for this student
                q_embed = model.q_embed(q_seq)  # [1, seq_len, embed_dim]
                
                # Use the last time step to get final ability
                final_q = q_seq[:, -1]  # [1]
                final_r = r_seq[:, -1]  # [1]
                
                # Get final attention and ability
                qa_input = torch.stack([final_q.float(), final_r.float()], dim=1)
                qa_embed = model.qa_embed(qa_input)
                query_key = model.q_to_key(q_embed[:, -1])
                
                attention_weights = model.compute_attention(query_key, final_q)
                ability_mean, _ = model.bayesian_read(attention_weights)
                
                # Extract final student ability (NO activation)
                final_ability = torch.sum(ability_mean * model.ability_weights, dim=1)
                student_abilities.append(final_ability.item())
    
    return np.array(student_abilities)


def compute_parameter_recovery_metrics(true_params: Dict[str, np.ndarray],
                                     learned_params: Dict[str, torch.Tensor],
                                     student_abilities: np.ndarray) -> Dict[str, float]:
    """
    Compute IRT parameter recovery metrics with proper theta comparison.
    """
    metrics = {}
    ...
    # Alpha comparison (memory slot alphas vs true question alphas)
    ...
    # Beta comparison (memory slot betas vs true question betas)
    ...
    # Theta comparison (FIXED: actual student abilities vs true student abilities)
    n_students = min(len(true_params['theta']), len(student_abilities))
    if n_students > 0:
        theta_corr = np.corrcoef(true_params['theta'][:n_students], 
                               student_abilities[:n_students])[0, 1]
        ...
        metrics['theta_mse'] = np.mean((true_params['theta'][:n_students] - 
                                      student_abilities[:n_students]) ** 2)
    else:
...
def main():
    parser = argparse.ArgumentParser(description='Train No-Activation Deep Bayesian-DKVMN')
    ...
    print("🚫 NO ACTIVATION FUNCTIONS for alpha, beta, theta parameters")
    ...
    for epoch in range(args.epochs):
        ...
        if test_metrics['categorical_accuracy'] > best_test_accuracy:
            ...
            save_path = save_dir / f'best_no_activation_{args.dataset}.pth'
    ...
    checkpoint = torch.load(save_dir / f'best_no_activation_{args.dataset}.pth')
    model.load_state_dict(checkpoint['model_state_dict'])
    learned_params = checkpoint['learned_params']
    
    # Extract actual student abilities (FIXED theta comparison)
    print("\nExtracting actual student abilities...")
    student_abilities = extract_student_abilities(model, test_loader, device)
    print(f"Extracted {len(student_abilities)} student abilities")
    
    # Compare with true parameters if available
    if true_params is not None:
        print("\nComparing learned vs true IRT parameters...")
        comparison_metrics = compute_parameter_recovery_metrics(true_params, learned_params, student_abilities)
        
        print("\n🎯 IRT Parameter Recovery Metrics (NO ACTIVATIONS):")
        ...
        history['student_abilities'] = student_abilities.tolist()
    
    # Save training history
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    history_file = log_dir / f'no_activation_training_history_{args.dataset}_{timestamp}.json'
