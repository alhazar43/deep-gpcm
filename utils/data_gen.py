...
from utils.data_utils import parse_dataset_name
...
class GpcmGen:
    ...
    def save_oc_format(self, sequences, data_dir, split_ratio=0.8):
        ...
        metadata = {
            ...
            'format': 'OC',
            'description': f'Synthetic GPCM data with {self.n_cats} ordered categories',
    ...
    def save_new_format(self, sequences, data_dir, dataset_name, split_ratio=0.8):
        """Save data in new format for synthetic_<students>_<max_questions>_<categories>."""
        data_dir = Path(data_dir)
        data_dir.mkdir(parents=True, exist_ok=True)
        ...
        # Save in OC format (integer categories)
        for split_name, split_seqs in [('train', train_seqs), ('test', test_seqs)]:
            filename = data_dir / f"{dataset_name}_{split_name}.txt"
        ...
        # Parse dataset name for metadata
        ...
        metadata = {
            ...
            'response_type': 'ordered_categorical',
            'format': 'new_format',
            'description': f'Synthetic GPCM data: {self.n_students} students, {self.n_questions} questions, {self.n_cats} categories',
            'train_students': n_train,
            'test_students': len(sequences) - n_train,
            'seq_len_range': self.seq_len_range
        }
        ...
        print(f"Saved new format dataset to {data_dir}")
    ...
    def gen_and_save(self, base_data_dir="./data", formats=["OC", "PC"], dataset_name=None):
        ...
        # If dataset_name provided, use new format
        if dataset_name:
            data_dir = Path(base_data_dir) / dataset_name
...
def main():
    ...
    # Parse parameters from dataset name if provided
    if args.name:
        params = parse_dataset_name(args.name)
        if params['format'] != 'new':
            print(f"Error: Invalid dataset name format: {args.name}")
            print("Expected format: synthetic_<students>_<max_questions>_<categories>")
            print("Example: synthetic_4000_200_2")
            return
        
        # Override parameters from dataset name
        n_students = params['n_students']
        max_questions = params['max_questions']
        n_cats = params['n_cats']
        
        # Generate question range: 100 to max_questions
