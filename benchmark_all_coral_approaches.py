#!/usr/bin/env python3
"""
Comprehensive Benchmark: GPCM vs Original CORAL vs Improved CORAL
...
def evaluate_model_comprehensive(model, data_loader, loss_fn, metrics, device, n_cats, 
    ...
    with torch.no_grad():
        for q_batch, r_batch, mask_batch in data_loader:
            ...
            if model_type == "improved_coral":
                # Improved CORAL model
                _, _, _, probs, coral_logits = model(q_batch, r_batch, use_coral=True)
            elif model_type == "original_coral":
                # Original CORAL model
            ...
            # Compute loss
            if model_type == "improved_coral" and trainer is not None:
...
def train_single_model(dataset_name, embedding_strategy, model_type, n_epochs=15, device=None):
    """Train a single model (GPCM, original CORAL, or improved CORAL)."""
    if device is None:
    ...
    if model_type == "gpcm":
        ...
        loss_fn = OrdinalLoss(n_cats)
        print("Created original CORAL model")
    elif model_type == "improved_coral":
        model = create_improved_coral_enhanced_model(base_model).to(device)
        trainer = ImprovedCoralTrainer(model, n_cats, device)
        loss_fn = None  # Trainer handles loss
        print("Created improved CORAL model")
    else:
    ...
    for epoch in range(n_epochs):
        ...
        train_loss = train_epoch_comprehensive(
            model, train_loader, optimizer, loss_fn, device, n_cats, model_type, trainer
        )
        
        # Evaluate
        train_metrics = evaluate_model_comprehensive(
            model, train_loader, loss_fn, metrics, device, n_cats, model_type, trainer
        )
        test_metrics = evaluate_model_comprehensive(
            model, test_loader, loss_fn, metrics, device, n_cats, model_type, trainer
        )
...
def run_comprehensive_benchmark(dataset_name="synthetic_OC", epochs=15):
    """
    Run comprehensive benchmark: GPCM vs Original CORAL vs Improved CORAL.
    """
    print(f"\n{'='*100}")
    print(f"COMPREHENSIVE 3-WAY BENCHMARK")
    print(f"GPCM vs Original CORAL vs Improved CORAL")
    print(f"Dataset: {dataset_name}")
    print(f"{'='*100}")
    ...
    model_types = ['gpcm', 'original_coral', 'improved_coral']
    ...
    with open(detailed_path, 'w') as f:
        json.dump({
            'experiment_info': {
                ...
                'description': 'Comprehensive 3-way benchmark: GPCM vs Original CORAL vs Improved CORAL'
            },
            'results': all_results
        }, f, indent=2)
